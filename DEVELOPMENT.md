...
Picker layout work (2026-09-25): the native `OpenCode (<directory>)` title remains intact. On macOS, a session title leads the native body when available, without duplicating a configured message that already contains it; an actionable terminal-notifier alert still ends in its focus token. The Hammerspoon chooser now puts `(<directory>) · <session title>` on the primary row and `OpenCode · <event>` plus relative time below it. A compact empty-query placeholder carries action hints. The toolbar experiment and its live focus behavior are recorded in `~/.hammerspoon/docs/hammerspoon-interactions.org`. Source tests/typecheck/build passed; the shared server restarted after the build, `opencode service status` returned its local URL, and `opencode plugin list` contained `opencode-notifier`. A disposable native alert showed the session title before its event, and a full-picker capture verified the 44%-width centered layout and readable hint on the primary display. Existing TUI pane leases require TUI reconnection; a fresh token-bearing production alert remains a separate follow-up check.
